...
def main():
    ...
    if actor_files:
        latest_actor = max(actor_files, key=lambda f: int(re.search(r'actor_(\d+)', f).group(1)))
        latest_critic = latest_actor.replace('actor_', 'critic_')
        start_episode = int(re.search(r'actor_(\d+)', latest_actor).group(1))
    ...
    try:
        for ep in range(start_episode, episodes):
            ...
            # ==========================================
            # A perfect run generates ~490 to 520 reward. Anything over 450 means it hit the +300 success trigger.
            if ep_reward >= 450.0:
                consecutive_successes += 1
                print(f"\n🎉 AMAZING! The drone reached the target perfectly! (Streak: {consecutive_successes}/3)")
                
                # Immediately save this winning model so we don't lose it
                ...
                if consecutive_successes >= 3:
                    print("\n✅ GOAL MET: The model has successfully reached the target 3 times in a row!")
                    print("Stopping training early to prevent Catastrophic Forgetting. You are ready to test!")
                    break  # This cleanly exits the training loop
            else:
                consecutive_successes = 0  # Reset streak if it crashes or times out
